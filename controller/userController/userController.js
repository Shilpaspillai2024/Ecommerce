...
const signpost = async (req, res, next) => {
  try {
    ...
    if (userExist) {
      req.flash(
        "errorMessage",
        "An account with this email address already exsist,pls try with another email ! "
      );
      res.redirect("/signup");
    } else {
      ...
      req.session.otp = otp;
      req.session.otpExpireTime = Date.now() + 2 * 60 * 1000;
...
const otp = (req, res, next) => {
  try {
    console.log(req.session.otpExpireTime);
    res.render("user/OTP", {
      title: "OTP verification",
      emailAddress: req.session.email,
      alertMessage: req.flash("errorMessage"),
      otpExpireTime: req.session.otpExpireTime,
      user: req.session.user,
    });
  } catch (err) {
...
const otpPost = async (req, res, next) => {
  try {
    if (req.session.otp !== undefined) {
      if (Date.now() > req.session.otpExpireTime) {
        req.flash("errorMessage", "OTP expired! Please resend OTP.");
      ...
      req.flash(
        "errorMessage",
        "An error OCCured during otp genetation try again !"
      );
      res.redirect("/signup");
    }
  } catch (err) {
...
const otpResend = (req, res, next) => {
  try {
    if (req.session.otpExpireTime && Date.now() < req.session.otpExpireTime) {
      req.flash("errorMessage", "Please wait — OTP still active!");
      return res.redirect("/otp");
    }
